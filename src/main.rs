...
async fn run_main(args: Args) -> Result<()> {
    ...
    let (mut cluster, connect_error) = if context_picker {
        if args.context.is_some() {
            let labels: Vec<String> = contexts.iter().map(|e| e.label.clone()).collect();
            args.validate_picker_context(&labels)?;
        }
        (Cluster::disconnected(requested.as_ref()), None)
    } else {
        eprintln!("Connecting to cluster…");
        let connect = match &requested {
            Some(id) => {
                Cluster::connect_context(id, args.allow_v1_client_cert, args.no_tls_resumption)
                    .await
            }
