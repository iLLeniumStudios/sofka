...
impl App {
    ...
    pub(super) fn switch_context(&mut self, id: crate::kubeconfigs::ClusterId) {
        ...
        tokio::spawn(async move {
            let result = Cluster::connect_context(&id, allow_v1_client_cert, no_tls_resumption)
                .await
                .map(Box::new)
    ...
    pub(super) fn apply_context_switch(
        ...
        self.namespace = self
            .launch_namespace
            .take()
            .or_else(|| self.namespace_memory.get(&id.state_key()))
    ...
    pub(super) fn key_kubeconfigs(&mut self, key: KeyInput) {
        let len = self.kubeconfig_rows.len() + 1;
        match (key.action, key.code) {
            (Some(Action::Back), _) | (Some(Action::Close), _) => self.mode = Mode::Table,
            (Some(Action::Down), _) => list_step(&mut self.kubeconfig_state, len, true),
            (Some(Action::Up), _) => list_step(&mut self.kubeconfig_state, len, false),
            (Some(Action::AddSource), _) => {
                self.prompt_label = "Add kubeconfig file or directory:".into();
                self.prompt_input.clear();
                self.prompt_kind = Some(PromptKind::AddKubeconfig);
                self.mode = Mode::Prompt;
            }
            (Some(Action::Delete), _) => self.remove_selected_kubeconfig(),
            // Enter goes where you were headed anyway: the contexts these
            // files provide.
            (Some(Action::Accept), _) => self.open_contexts(),
            _ => {}
        }
    }
    ...
    fn remove_selected_kubeconfig(&mut self) {
        ...
        // Contexts that source provided are gone; the completion cache must
        // stop offering them.
        self.refresh_context_cache();
        self.rebuild_kubeconfig_rows();
        let selected = self.kubeconfig_state.selected().unwrap_or(0);
